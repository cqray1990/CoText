...
def parse_xml(annotation_path,image_path):
    ...
    for idx,child in enumerate(root):
        bboxes,IDs = getBboxesAndLabels_icd13(height, width, child)
...
from_image_root = '/home/wangjue_Cloud/wuweijia/Data/ICDAR2015_video/train/frames'
from_label_root = '/home/wangjue_Cloud/wuweijia/Data/ICDAR2015_video/train/gt'

seqs = [s for s in os.listdir(from_image_root)]
seq_label_root = "/home/wangjue_Cloud/wuweijia/Data/ICDAR2015_video/icd15/train_gt"
seq_image_root = "/home/wangjue_Cloud/wuweijia/Data/ICDAR2015_video/icd15/train_image"
...
for seq in tqdm(seqs):
    ...
    image_path_frame = osp.join(from_image_root,seq)
...
#     print(seq)
    ...
    bboxess,IDss = parse_xml(ann_path,osp.join(image_path_frame,os.listdir(image_path_frame)[0]))
    
    ID_list = {}
    
    for i in range(len(bboxess)):
        frame_id = i
        label_fpath = osp.join(seq_label_root, '{}.txt'.format(image_id))
        frame_fpath = osp.join(seq_image_root,"{}.jpg".format(image_id))
        
        frame_path_one = osp.join(image_path_frame,"{}.jpg".format(frame_id+1))
#         print
        ...
        with open(label_fpath, 'w') as f:
            if bboxess[i] is None:
                continue
#             print(bboxess[i])
            for bboxes,ids in zip(bboxess[i],IDss[i]):
                strResult = ','.join(
                    [str(bboxes[0]), str(bboxes[1]), str(bboxes[2]), str(bboxes[3]), str(bboxes[4]), str(bboxes[5]),
                     str(bboxes[6]), str(bboxes[7]), str(bboxes[8]), str(ids)]) + '\r\n'
