...
class EmbLoss_v2(nn.Module):
    ...
    def forward_single(self, emb, instance, kernel, training_mask, bboxes):
        ...
        for i, lb in enumerate(unique_labels):
            if lb == 0:
                continue
            ind_k = instance_kernel == lb  # 该实例的索引
            emb_mean[:, i] = torch.mean(emb[:, ind_k], dim=1) # 为该实例的特征值均值
        ...
        for i, lb in enumerate(unique_labels):
            ...
            ind = instance == lb
            emb_ = emb[:, ind]   # 该实例的
            dist = (emb_ - emb_mean[:, i:i + 1]).norm(p=2, dim=0) # 平方和开根号，四点求
        ...
        loss = l_agg + l_dis + l_reg  # l_reg是用来限制emb的模长不能太大，有没有这一项估计差别不是很大。
        return loss
