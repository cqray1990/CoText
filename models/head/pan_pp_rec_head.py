...
#                 mask = (instance_[:, t:b, l:r] == label).float()
    ...
    def loss(self, input, target, rec_masks=None, reduce=True):
        EPS = 1e-6
        if torch.sum(rec_masks) == 0:
            return {'loss_rec': input.new_full((1,), 0, dtype=torch.float32),
                    'acc_rec': input.new_full((1,), 0, dtype=torch.float32)}
        input = input[rec_masks==1, :, :]
        target = target[rec_masks==1, :]
        N, L, D = input.size()
        ...
        loss_rec = torch.sum(loss_rec * mask.float(), dim=1) / (torch.sum(mask.float(), dim=1) + EPS)
        acc_rec = acc(torch.argmax(input, dim=1).view(N, L), target.view(N, L), mask, reduce=False)
        if reduce:
    ...
    def forward(self, x, target=None):
        ...
        if self.training:
            ...
            if self.beam_size <= 1:
                ...
                return self.decoder.beam_search(x, holistic_feature, beam_size=self.beam_size)
...
class Encoder(nn.Module):
    ...
    def __init__(self, hidden_dim, voc, char2id, id2char):
        ...
        self.emb = nn.Embedding(self.vocab_size, self.hidden_dim)  # 一个向量编码器
...
class Decoder(nn.Module):
    ...
    def __init__(self, featrue_dim, hidden_dim, num_layers, voc, char2id, id2char):
        ...
        self.att = MultiHeadAttentionLayer(self.hidden_dim, 8)
        self.cls = nn.Linear(self.hidden_dim + self.featrue_dim, self.vocab_size)
    ...
    def forward(self, x, holistic_feature, target):
        ...
        h = []
        for i in range(self.num_layers):
            h.append((x.new_zeros((x.size(0), self.hidden_dim), dtype=torch.float32),
                      x.new_zeros((x.size(0), self.hidden_dim), dtype=torch.float32)))

        out = x.new_zeros((x.size(0), max_seq_len + 1, self.vocab_size), dtype=torch.float32)
        for t in range(max_seq_len + 1):
            if t == 0:
                xt = holistic_feature
            elif t == 1:
                it = x.new_full((batch_size,), self.START_TOKEN, dtype=torch.long)
                xt = self.emb(it)
            else:
                it = target[:, t - 2]
                xt = self.emb(it)
    ...
    def to_words(self, seqs, seq_scores=None, out = None):
        EPS = 1e-6
        words = []
        word_scores = None
        ...
        out_zero = seqs.new_zeros((out.size(0), out.size(1), self.vocab_size), dtype=torch.float32)
    ...
    def forward_test(self, x, holistic_feature):
        ...
        max_seq_len = 32
        out = x.new_zeros((x.size(0), max_seq_len + 1, self.vocab_size), dtype=torch.float32)
        seq = x.new_full((batch_size, max_seq_len + 1), self.START_TOKEN, dtype=torch.long)
        seq_score = x.new_zeros((batch_size, max_seq_len + 1), dtype=torch.float32)
        ...
        words, word_scores,out_zero = self.to_words(seq[:, 1:], seq_score[:, 1:],out[:, 1:, :])
#         print(words)
#         print(out[:, 1:, :].shape)
        return words, word_scores, out_zero
    ...
    def beam_search(self, x, holistic_feature, beam_size=2):
        ...
        init_inputs = x.new_full((batch_size,), self.START_TOKEN, dtype=torch.long)
