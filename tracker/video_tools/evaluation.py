...
from video_parser import VideoTool
import re, string
...
class Evaluator(object):
    ...
    def eval_video(self, gt_track_objs, pre_track_objs, ignore_mode=False):
        ...
        similarity_mat = np.zeros((num_gt, num_pre))
            ...
            for j in range(num_pre):
                ...
                if tiou_mat[i][j] >= self.tiou_thresh:
                    if not ignore_mode:
                        ...
                        piou_mat[i][j] = self.cal_piop(gt_object['tracks'], pre_object['tracks'])
                    similarity_mat[i][j] = self.cal_similarity(gt_object['text'], pre_object['text'])
            ...
            for j in range(num_pre):
                if not ignore_mode:
                    if gt_match_mat[i] == 0 and pre_match_mat[j] == 0:
                        if tiou_mat[i][j] > self.tiou_thresh and piou_mat[i][j] > self.piou_thresh and similarity_mat[i][j] > self.sim_thresh:
                            # 匹配成功
                            gt_match_mat[i] = 1
                            pre_match_mat[j] = 1
    ...
    def cal_piop(self, frame_dict1, frame_dict2):
        inter_ids = frame_dict1.keys() & frame_dict2.keys()
        piou = 0
        for frame_id in inter_ids:
            box1 = frame_dict1[frame_id]
            box2 = frame_dict2[frame_id]
            piou += self.cal_iop_poly(box1[:8], box2[:8])
        return piou / len(inter_ids)
    ...
    def cal_iop_poly(bbox1, bbox2):
        ...
        if not poly1.intersects(poly2):
            ...
            inter_area = poly1.intersection(poly2).area
            iop = float(inter_area) / poly2.area
        return iop
...
if __name__ == '__main__':
    ...
    if args.vis:
        # 可视化分析
        videoTool = VideoTool()
        frame_dir = args.frame_dir      # 存放原始抽帧的结果图片
        out_dir = args.out_dir          # 可视化结果输出路径
        for video_id in evaler.video_id_list:
            img_dir = osp.join(frame_dir, video_id)
            vis_dir = osp.join(out_dir, video_id)
            # 画TP
            videoTool.frames_tracking(img_dir=img_dir, 
                                      out_dir=vis_dir,
                                      track_list=evaler.register[video_id]["TP"],
                                      keyword='TP')
            # 画FP
            videoTool.frames_tracking(img_dir=vis_dir, 
                                      out_dir=vis_dir,
                                      track_list=evaler.register[video_id]["FP"],
                                      keyword='FP')

            # 画FN
            videoTool.frames_tracking(img_dir=vis_dir, 
                                      out_dir=vis_dir,
                                      track_list=evaler.register[video_id]["FN"],
                                      keyword='FN')
            # 画IGNORE
            videoTool.frames_tracking(img_dir=vis_dir, 
                                      out_dir=vis_dir,
                                      track_list=evaler.register[video_id]["IGNORE"],
                                      keyword='IGNORE')

            videoTool.imgs2video(frame_dir=vis_dir, video_path=osp.join(vis_dir, video_id+'.mp4'), rate=2)
