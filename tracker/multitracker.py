...
#             print(dists1)
            ...
            matches, u_track, u_detection, min_cost = matching.linear_assignment(dists1, thresh=self.config["embedding_distance_thresh"])   # 匹配结果、未匹配轨迹、未匹配检测
...
#             f2.close()
        ...
        if self.config["perform_iou"]:
            ...
            matches, u_track, u_detection,_ = matching.linear_assignment(dists, thresh=self.config["IOU_distance_thresh"]) # IOU > 0.7认为是匹配
            for itracked, idet in matches:
...
#                 print("=".join(["IOU合并", str(dists[itracked][idet]), track.content, det.content, str(Levenshtein.ratio(track.content, det.content))]))
        ...
        for i in range(len(detections)):
            ...
            track = detections[i]
            if self.config["perform_kalman"]:
                track.activate(self.frame_id, kalman_filter=self.kalman_filter)
            else:
