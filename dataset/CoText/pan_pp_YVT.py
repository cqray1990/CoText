...
# YVT_video_train_data_dir = '/home/wangjue_Cloud/wuweijia/Data/VideoText/YVT/icd15/train_image'
# YVT_video_train_gt_dir = '/home/wangjue_Cloud/wuweijia/Data/VideoText/YVT/icd15/train_gt'

YVT_video_train_data_dir = '/new_share/wuweijia/Data/VideoText/YVT/icd15/train_image'
YVT_video_train_gt_dir = '/new_share/wuweijia/Data/VideoText/YVT/icd15/train_gt'


class PAN_YVT_video(data.Dataset):
    def __init__(self,
                 split='train',
                 is_transform=False,
                 img_size=None,
        ...
        self.img_paths['YVT'] = []
        self.gts['YVT'] = []
        img_names = [img_name for img_name in mmcv.utils.scandir(YVT_video_train_data_dir, '.jpg')]
        img_names.extend([img_name for img_name in mmcv.utils.scandir(YVT_video_train_data_dir, '.png')])
        for idx, img_name in enumerate(img_names):
            img_path = osp.join(YVT_video_train_data_dir, img_name)
            self.img_paths['YVT'].append(img_path)

            gt_name = img_name.split('.')[0] + '.txt'
            gt_path = osp.join(YVT_video_train_gt_dir, gt_name)
            self.gts['YVT'].append(gt_path)
            
        self.img_paths['YVT'] = self.img_paths['YVT'] * 5
        self.gts['YVT'] = self.gts['YVT'] * 5
        self.img_num += len(self.img_paths['YVT'])
