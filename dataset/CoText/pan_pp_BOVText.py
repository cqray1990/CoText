...
ic15_train_data_dir = '/share/wuweijia/Data/ICDAR2015/train_image/'
ic15_train_gt_dir = '/share/wuweijia/Data/ICDAR2015/train_gt/'
...
# /new_share/  home/wangjue_Cloud
BOVText_video_train_data_dir = '/home/wangjue_Cloud/wuweijia/MyBenchMark/MMVText/BOVTextV2/icd15/train_image'
BOVText_video_train_gt_dir = '/home/wangjue_Cloud/wuweijia/MyBenchMark/MMVText/BOVTextV2/icd15/train_gt'
...
class PAN_BOVText_video(data.Dataset):
    def __init__(self,
                 split='train',
                 is_transform=False,
                 img_size=None,
...
#         self.img_paths['cocotext'] = self.img_paths['cocotext'] * 5
#         self.gts['cocotext'] = self.gts['cocotext'] * 5
...
#         self.img_paths['lsvt'] = self.img_paths['lsvt'] * 10
#         self.gts['lsvt'] = self.gts['lsvt'] * 10
...
#         self.img_paths['mtwi'] = self.img_paths['mtwi'] * 10
#         self.gts['mtwi'] = self.gts['mtwi'] * 10
#         self.img_num += len(self.img_paths['mtwi'])
        
        # BOVText
        self.img_paths['bovtext_video'] = []
        self.gts['bovtext_video'] = []
        img_names = [img_name for img_name in mmcv.utils.scandir(BOVText_video_train_data_dir, '.jpg')]
        img_names.extend([img_name for img_name in mmcv.utils.scandir(BOVText_video_train_data_dir, '.png')])
        for idx, img_name in enumerate(img_names):
            img_path = osp.join(BOVText_video_train_data_dir, img_name)
            self.img_paths['bovtext_video'].append(img_path)

            gt_name = img_name.split('.')[0] + '.txt'
            gt_path = osp.join(BOVText_video_train_gt_dir, gt_name)
            self.gts['bovtext_video'].append(gt_path)
            
        self.img_paths['bovtext_video'] = self.img_paths['bovtext_video']
        self.gts['bovtext_video'] = self.gts['bovtext_video']
        self.img_num += len(self.img_paths['bovtext_video'])
        
        self.voc, self.char2id, self.id2char = get_vocabulary('CHINESE', use_ctc=self.use_ctc)
        self.max_word_num = 50
        self.max_word_len = 62
...
#         print(dataset_cat, img_path, gt_path)
        if dataset_cat == 'bovtext_video':
