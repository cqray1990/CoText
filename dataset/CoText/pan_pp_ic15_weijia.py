...
ic15_video_train_data_dir = '/home/wangjue_Cloud/wuweijia/Data/ICDAR2015_video/icd15/train_image' #home/wangjue_Cloud
ic15_video_train_gt_dir = '/home/wangjue_Cloud/wuweijia/Data/ICDAR2015_video/icd15/train_gt'   # home/wangjue_Cloud

# ic15_video_train_data_dir = '/share/lizhuang05/datasets/ICDAR2015_VIDEO/train_images'
# ic15_video_train_gt_dir = '/share/lizhuang05/datasets/ICDAR2015_VIDEO/train_gts'
...
#         print(img_names)
        ...
        self.img_paths['ic15_video'] = self.img_paths['ic15_video']
        self.gts['ic15_video'] = self.gts['ic15_video'] #3
...
#             img, bboxes, words, img_path, track_ids = self.load_ic15_video_single(img_path, gt_path)
...
#             img, bboxes, words, img_path = self.load_synthtext_single(img_path, gt_path)
        ...
        for j in range(self.num_frames):
            index_ = index - inds[j]
            dataset_cat, img_path, gt_path = self.data_list[index_]
            img, bboxes, words, img_path, track_ids = self.load_ic15_video_single(img_path, gt_path)
#             print(img_path)
            for i,ids in enumerate(track_ids):
#                 if id_new > self.max_word_num:
#                     continue
                if ids not in track_map:
                    track_map[ids] = id_new
                    id_new += 1
                    
            h, w = img.shape[0:2]
            bboxes = [np.array(bbox) / ([w * 1.0, h * 1.0] * 4) for bbox in bboxes]
            bboxes = np.array(bboxes)
            data1 = self.__generate_data(copy.deepcopy(img), copy.deepcopy(bboxes), words.copy(), "same", track_ids, track_map)
            final_data.append(data1)
        return final_data
    
#             if index>1:
#                 dataset_cat1, img_path1, gt_path1 = self.data_list[index-1]
#                 if dataset_cat1 == "ic15_video":
#                     img1, bboxes1, words1, img_path1, track_ids1 = self.load_ic15_video_single(img_path1, gt_path1)
                    
#                     new_track_ids = track_ids1+ track_ids

#                     track_map = {}
#                     id_new = 1
#                     for i,ids in enumerate(new_track_ids):
#                         if ids not in track_map:
#                             track_map[ids] = id_new
#                             id_new += 1
...
#                     bboxes = [np.array(bbox) / ([w * 1.0, h * 1.0] * 4) for bbox in bboxes]
#                     bboxes = np.array(bboxes)
#                     data1 = self.__generate_data(copy.deepcopy(img), copy.deepcopy(bboxes), words.copy(), img_path, track_ids, track_map)
                    
#                     h, w = img1.shape[0:2]
#                     bboxes1 = [np.array(bbox) / ([w * 1.0, h * 1.0] * 4) for bbox in bboxes1]
#                     bboxes1 = np.array(bboxes1)
#                     data2 = self.__generate_data(copy.deepcopy(img1), copy.deepcopy(bboxes1), words1.copy(), img_path, track_ids1, track_map)
...
#         return [data1, data2]
    ...
    def __generate_data(self, img, bboxes, words, img_path, tracking_id=None, track_map=None):

        if len(bboxes) > self.max_word_num:
            bboxes = bboxes[:self.max_word_num]
            words = words[:self.max_word_num]
            if tracking_id!=None:
                tracking_id = tracking_id[:self.max_word_num]
        ...
        for i, word in enumerate(words):
            track_id = tracking_id[i]
            
            desc_mask[int(track_map[track_id])] = 1  # desc_mask无需关心看不清的文字
            ...
            gt_words[int(track_map[track_id])] = gt_word
            word_mask[int(track_map[track_id])] = 1
...
#                     try:
                    track_id = tracking_id[i]
                    cv2.drawContours(gt_instance, [bboxes[i]], -1, int(track_map[track_id]), -1)
#                     except:
#                         continue
                else:
    ...
    def load_ic15_video_single(self, img_path, gt_path):
        img = get_img(img_path, self.read_type)
        bboxes, words,track_id = get_ann_ic15_video(img, gt_path)
        return img, bboxes, words, img_path, track_id
...
#             continue
        ...
        for i, data in enumerate(data_batch):
            visulization.visual_feature(
                        out_path=osp.join('./show/', img_id+'_heamap{}.jpg'.format(i)),
