...
ic15_root_dir = '/share/lizhuang05/datasets/OCR_eval/'
ic15_train_data_dir = ic15_root_dir + 'images/'
ic15_train_gt_dir = ic15_root_dir + 'gts/'

lsvt_root_dir = '/share/lizhuang05/datasets/LSVT/'
lsvt_train_data_dir = lsvt_root_dir + 'images/'
lsvt_train_gt_dir = lsvt_root_dir + 'gts/'

mtwi_root_dir = '/share/lizhuang05/datasets/MTWI/'
mtwi_train_data_dir = mtwi_root_dir + 'images/'
mtwi_train_gt_dir = mtwi_root_dir + 'gts/'

kwai_root_dir = '/share/lizhuang05/datasets/OCR_train/'
kwai_train_data_dir = kwai_root_dir + 'images/'
kwai_train_gt_dir = kwai_root_dir +  'gts/'

kwai_det_list = '/share/lizhuang05/datasets/DB/img_list_all4panpp_withrec.txt'
width_img_list = '/share/lizhuang05/datasets/DB/img_list_width_img_withrec.txt'
long_text_list = '/share/lizhuang05/datasets/DB/img_list_long_text_withrec.txt'
...
class PAN_PP_jointTrain(data.Dataset):
    def __init__(self,
        ...
        self.img_paths['lsvt'] = []
        self.gts['lsvt'] = []
        img_names = [img_name for img_name in mmcv.utils.scandir(lsvt_train_data_dir, '.jpg')]
        img_names.extend([img_name for img_name in mmcv.utils.scandir(lsvt_train_data_dir, '.png')])
        for idx, img_name in enumerate(img_names):
            img_path = lsvt_train_data_dir + img_name
            self.img_paths['lsvt'].append(img_path)

            gt_name = img_name.split('.')[0] + '.txt'
            gt_path = lsvt_train_gt_dir + gt_name
            self.gts['lsvt'].append(gt_path)
        self.img_num += len(self.img_paths['lsvt'])

        # mtwi
        self.img_paths['mtwi'] = []
        self.gts['mtwi'] = []
        img_names = [img_name for img_name in mmcv.utils.scandir(mtwi_train_data_dir, '.jpg')]
        img_names.extend([img_name for img_name in mmcv.utils.scandir(mtwi_train_data_dir, '.png')])
        for idx, img_name in enumerate(img_names):
            img_path = mtwi_train_data_dir + img_name
            self.img_paths['mtwi'].append(img_path)

            gt_name = ".".join(img_name.split('.')[:-1]) + '.txt'
            gt_path = mtwi_train_gt_dir + gt_name
            self.gts['mtwi'].append(gt_path)
        self.img_num += len(self.img_paths['mtwi'])

        # kwai
        self.img_paths['kwai'] = []
        self.gts['kwai'] = []
        img_names = [img_name for img_name in mmcv.utils.scandir(kwai_train_data_dir, '.jpg')]
        img_names.extend([img_name for img_name in mmcv.utils.scandir(kwai_train_data_dir, '.png')])
        for idx, img_name in enumerate(img_names):
            img_path = kwai_train_data_dir + img_name
            self.img_paths['kwai'].append(img_path)

            gt_name = ".".join(img_name.split('.')[:-1]) + '.txt'
            gt_path = kwai_train_gt_dir + gt_name
            self.gts['kwai'].append(gt_path)
        self.img_num += len(self.img_paths['kwai'])
        
        # kwai_det
        self.img_paths['kwai_det'] = []
        self.gts['kwai_det'] = []
        with open(kwai_det_list, 'r', encoding='utf-8') as f:
            for line in f.readlines():
                img_path = line.strip()
                gt_path = img_path.replace('train_images', 'train_gts_withrec').replace('.jpg', '.txt')
                self.img_paths['kwai_det'].append(img_path)
                self.gts['kwai_det'].append(gt_path)
        self.img_num += len(self.img_paths['kwai_det'])

        # width_img
        self.img_paths['width_img'] = []
        self.gts['width_img'] = []
        with open(width_img_list , 'r', encoding='utf-8') as f:
            for line in f.readlines():
                img_path = line.strip()
                gt_path = img_path.replace('train_images', 'train_gts_withrec').replace('.jpg', '.txt')
                self.img_paths['width_img'].append(img_path)
                self.gts['width_img'].append(gt_path)
        self.img_num += len(self.img_paths['width_img'])

        # long_text_list
        self.img_paths['long_text'] = []
        self.gts['long_text'] = []
        with open(long_text_list, 'r', encoding='utf-8') as f:
            for line in f.readlines():
                img_path = line.strip()
                gt_path = img_path.replace('train_images', 'train_gts_withrec').replace('.jpg', '.txt')
                self.img_paths['long_text'].append(img_path)
                self.gts['long_text'].append(gt_path)
        self.img_num += len(self.img_paths['long_text'])
    ...
    def __getitem__(self, index): 
        ...
        if choice < 4.0 / 100:  
            # 26584 4%
            index = random.randint(0, len(self.img_paths['lsvt']) - 1)
            img, bboxes, words, img_path = self.load_lsvt_single(index)
        elif choice < 7.0 / 100:
            # 9924 3%
            index = random.randint(0, len(self.img_paths['mtwi']) - 1)
            img, bboxes, words, img_path = self.load_mtwi_single(index)
        elif choice < 15 / 100:
            # 27685 8%
            ...
            # 9.6w 15%
            index = random.randint(0, len(self.img_paths['kwai']) - 1)
            img, bboxes, words, img_path = self.load_kwai_single(index)
        elif choice < 80.0 / 100:
            # 302277  30%
            index = random.randint(0, len(self.img_paths['kwai_det']) - 1)
            img, bboxes, words, img_path = self.load_kwai_det_single(index)
        elif choice < 90.0 / 100: 
            # 20%
            # 57694  w > h 图片
            index = random.randint(0, len(self.img_paths['width_img']) - 1)
            img, bboxes, words, img_path = self.load_width_img_single(index)
        else:  # if choice < 100 / 100:
            # 14720 /w >0.9的文本框
            # 20%
            index = random.randint(0, len(self.img_paths['long_text']) - 1)
            img, bboxes, words, img_path = self.load_long_text_single(index)
