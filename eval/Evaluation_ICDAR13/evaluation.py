...
def parse_args():
    ...
    parser.add_argument('--groundtruths', type=str,default='./eval/Evaluation_ICDAR13/gt'
                        , help='Directory containing ground truth files.')
    parser.add_argument('--tests', type=str,default='./outputs/pan_pp_r18_ICDAR15/json',
    ...
    def eval_file(self, filename):
        ...
        for frame_id in range(len(self.gt_frame_dict)):
            ...
            if str(frame_id) in result_frame_dict.keys():
                ...
                trk_ids = []
                for trk in trk_objs:
                    trk_tlwhs.append(np.array(trk["points"],dtype=np.float32))
                    trk_ids.append(np.array(trk["ID"],dtype=np.float32))
            else:
                trk_tlwhs = np.array([])
                trk_ids = np.array([])
